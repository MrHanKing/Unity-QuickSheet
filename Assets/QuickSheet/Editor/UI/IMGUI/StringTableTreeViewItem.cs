...
{
    // class StringTableTreeViewItem : GenericAssetTableTreeViewItem<ColumnDatas>
    // {
    //     SharedTableData m_SharedTableData;

    //     (SmartFormatField editor, ISelectable selected)[] m_TableProperties;

    //     public override void Initialize(ExcelMachine collection, int startIdx, List<ColumnDatas> sortedTables)
    //     {
    //         m_TableProperties = (SmartFormatField editor, ISelectable selected)[startIdx + sortedTables.Count];
