...
namespace ExcelEditor.Tool
{
    ...
    // 表格View
    // </summary>
    // <typeparam name="T1">编辑器表格数据</typeparam>
    // <typeparam name="T2">渲染行</typeparam>
        ...
        where T2 : GenericAssetTableTreeViewItem<T1>, new()
    {
        ...
        /// 表格名字
        /// </summary>
        protected string TableCollectionName => TableCollection.WorkTableName;

        /// <summary>
        /// 编辑器对应表格总数据
        /// </summary>
        /// <value></value>
        public ExcelMachine TableCollection { get; private set; }
        ...
        List<ColumnDatas> m_SortedTables = new List<ColumnDatas>();

        const string kSearch = "Localization-TablesView-Search";

        protected GenericAssetTableListView(ExcelMachine tableCollection) :
            base(new TreeViewState())
        {
            ...
            // ExcelSettings.Instance.editorEvents.TableEntryAdded += EditorEvents_TableEntryModified;
            // ExcelSettings.Instance.editorEvents.TableEntryRemoved += EditorEvents_TableEntryModified;
            // ExcelSettings.Instance.editorEvents.AssetTableEntryAdded += EditorEvents_AssetTableEntryAdded;
            // ExcelSettings.Instance.editorEvents.AssetTableEntryRemoved += EditorEvents_AssetTableEntryRemoved;
            // ExcelSettings.Instance.editorEvents.TableAddedToCollection += EditorEvents_TableAddedOrRemoveFromCollection;
            // ExcelSettings.Instance.editorEvents.TableRemovedFromCollection += EditorEvents_TableAddedOrRemoveFromCollection;
            // ExcelSettings.Instance.editorEvents.CollectionModified += EditorEvents_CollectionModified;
            // ExcelSettings.Instance.editorEvents.LocaleSortOrderChanged += EditorEvents_LocaleSortOrderChanged;
        ...
        void EditorEvents_CollectionModified(object sender, ExcelMachine collection)
        {
        ...
        /// <param name="collection"></param>
        /// <param name="entry"></param>
        void EditorEvents_TableEntryModified(ExcelMachine collection, RowEntryHeader entry)
        {
        ...
        // 列操作
        void EditorEvents_TableAddedOrRemoveFromCollection(ExcelMachine collection, ColumnHeader column)
        {
            if (collection == TableCollection)
            {
                Initialize();
        ...
        void EditorEvents_LocaleSortOrderChanged(object sender, ColumnHeader column)
        {
            InitializeColumns();
            Reload();
        }
        ...
        protected virtual void InitializeColumns()
        {
            ...
            var keys = TableCollection.allRowEntryHeader;
            ...
            {
                columns.Add(new TableColumn<T1>(TableCollection, table.asset, table.asset.columnHeader));
            }
        ...
        /// <param name="entry"></param>
        /// <returns></returns>
        protected virtual T2 CreateTreeViewItem(int index, RowEntryHeader entry)
        {
        ...
        protected override TreeViewItem BuildRoot()
        {
            ...
            if (TableCollection.allRowEntryHeader == null)
            {
                Debug.LogError($"No {nameof(AllRowEntryHeader)} assigned to Table: " + TableCollectionName);
                SetupParentsAndChildrenFromDepths(root, items);
                return root;
            }

            var sharedEntries = TableCollection.allRowEntryHeader.m_Entries;
        ...
        protected virtual void DrawKeyField(Rect cellRect, T2 keyItem)
        {
            ...
            if (EditorGUI.EndChangeCheck())
            {
                if (TableCollection.allRowEntryHeader.Contains(newKey))
                {
                ...
                {
                    // 修改行key
                    Undo.RecordObject(TableCollection.allRowEntryHeader, "Rename key");
                    TableCollection.allRowEntryHeader.RenameKey(keyItem.Key, newKey);
                    EditorUtility.SetDirty(TableCollection.allRowEntryHeader);
            ...
            {
                var objects = new UnityEngine.Object[TableCollection.Tables.Count + 1];
                for (int i = 0; i < TableCollection.Tables.Count; ++i)
                {
                    objects[i] = TableCollection.Tables[i].asset;
                }
                objects[TableCollection.Tables.Count] = TableCollection.allRowEntryHeader;
        ...
        protected override DragAndDropVisualMode HandleDragAndDrop(DragAndDropArgs args)
        {
            ...
            if (args.performDrop)
            {
                var sharedData = TableCollection.allRowEntryHeader;
                Undo.RecordObject(sharedData, "Move Key");

                // 移动换位置
                var newIndex = Mathf.Clamp(args.insertAtIndex, 0, sharedData.m_Entries.Count);
                sharedData.m_Entries.Insert(newIndex, sharedData.m_Entries[itemIndex]);
                if (newIndex <= itemIndex)
                    ++itemIndex;
                sharedData.m_Entries.RemoveAt(itemIndex);
